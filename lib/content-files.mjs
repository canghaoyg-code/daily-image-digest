...
import { contentImages, validateEdition } from "./editorial.mjs";
import { newsroomReport, productFingerprint, validateAcceptance } from "./newsroom.mjs";
...
export async function validateAssets(edition, publicRoot) {
  ...
  for (const item of edition.items ?? []) {
    let novel = false;
    for (const img of contentImages(item)) {
      placements++;
      try {
        if (/^https?:\/\//.test(img.path)) {
          const url = new URL(img.path);
          if (url.username || url.password) throw new Error("图片地址包含凭据");
          if (hashes.has(img.path)) warnings.push(`${item.id}：重复图片 ${img.path}（不重复计数）`);
          else { hashes.set(img.path, img.path); novel = true; }
          continue;
        }
...
export async function checkFile(path, options = {}) {
  ...
  // Existing issues remain immutable. New publications cannot bypass the desk
  // by leaving its field out; archived desk-backed issues keep their binding.
  if (options.forPublication || edition.newsroom && edition.status === "published") {
    const desk = newsroomReport(edition, options);
    report.errors.push(...desk.errors); report.warnings.push(...desk.warnings);
    try { report.errors.push(...validateAcceptance(edition, await productFingerprint(edition, resolve("."), Boolean(options.forPublication)), {...options, archival:!options.forPublication})); }
    catch (error) { report.errors.push(`成品绑定检查失败：${error.message}`); }
  }
