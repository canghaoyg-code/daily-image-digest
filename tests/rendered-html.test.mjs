...
test("server-renders the daily illustrated digest", async () => {
  ...
  assert.match(html, /“紫檀”停止回旋逼近华南/);
  assert.match(html, /多家门店残留液体检出敌敌畏/);
  assert.match(html, /天工机器人百米跑进 8.86 秒/);
  assert.match(html, /已故人士金融账户查询服务/);
  assert.match(html, /郑钦文重新在大满贯赛场赢球/);
  assert.match(html, /华为第二代三折叠/);
  ...
  assert.match(html, /从“约三百人”到“解除一百零七人”/);
  assert.match(html, /同一热点的代表性观点/);
  assert.match(html, /671 转发/);
  assert.match(html, /8.8 万浏览/);
  assert.match(html, /招录 440 名高校毕业生/);
  assert.ok((html.match(/查看这条观点/g) ?? []).length >= 7);
  assert.ok(html.indexOf("七种声音谈契约") < html.indexOf("“紫檀”停止回旋逼近华南"));

  assert.ok(html.indexOf("多家门店残留液体检出敌敌畏") < html.indexOf("天工机器人百米跑进 8.86 秒"));
  assert.ok(html.indexOf("美加贸易谈判破裂") < html.indexOf("一枚火箭上面级撞上月球"));
  assert.ok(html.indexOf("NASA 拍到了新月坑") < html.indexOf("无责救助者为何支付 1.9 万元"));
  assert.ok(html.indexOf("无责救助者为何支付 1.9 万元") < html.indexOf("只用字母、数字和符号"));
  assert.ok(html.indexOf("只用字母、数字和符号") < html.indexOf("29 个脚趾"));
});
