...
export function validateEdition(edition, { forPublication = false, now = new Date().toISOString() } = {}) {
  ...
  if (forPublication) {
    ...
    need(edition.editorialReview?.decision === "approved", "编辑验收未通过：明确 approved 后才可发布；技术检查不能代替选材复核");
  }
  if (!legacy) {
  ...
  for (const item of items) {
    ...
    if (!legacy) {
      ...
      for (const block of item.blocks ?? []) {
        ...
        if (block.kind === "quote") {
          need(block.text && sources.get(block.sourceId)?.author && references.includes(block.sourceId) && roles.includes(block.role), "原话块缺少发布者、条目内原文来源或角色：" + item.id);
          need(!block.presentation || ["excerpt", "translation", "paraphrase"].includes(block.presentation), "发言呈现方式无效：" + item.id);
        }
      ...
      for (const voice of item.voices ?? []) {
        ...
        need(!voice.presentation || ["excerpt", "translation", "paraphrase"].includes(voice.presentation), "发言呈现方式无效：" + item.id);
      }
